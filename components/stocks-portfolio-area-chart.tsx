...
export function StocksPortfolioAreaChart() {

  const supabase = createClient();

  const [stocksPortfolioSummary, setStocksPortfolioSummary] = React.useState<any[] | null>(null);

  React.useEffect(() => {
    const fetchStocksPortfolioSummary = async () => {
  ...
  return (
    <Card className="@container/card">
      <CardHeader>
        <CardTitle>Stocks Portfolio</CardTitle>
        <CardDescription>
          Historical Stocks Performance
        </CardDescription>
      </CardHeader>
      <CardContent className="px-2 pt-4 sm:px-6 sm:pt-6">
        {stocksPortfolioSummary ? (
          ...
          >
            <AreaChart data={stocksPortfolioSummary}>
              <defs>
